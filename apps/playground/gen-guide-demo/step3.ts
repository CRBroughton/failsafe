...
async function demo(label: string, email: string, password: string): Promise<void> {
  ...
  if (isErr(result)) {
    match(result.error, {
      InvalidCredentials: e => console.log("invalid credentials:", e.email),
      NetworkError: e => console.log("network error:", e.status, e.url),
      JSONStringifyError: () => console.log("could not serialize profile"),
      JSONParseError: e => console.log("could not parse profile:", e.raw),
    })
  }
  else {
